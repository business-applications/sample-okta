...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-okta-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-okta-service apps/sample-okta-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-okta-service:1.0-SNAPSHOT
    oc expose svc/sample-okta-service
else

	echo "Launching the application locally..."
	pattern="sample-okta-service"
	files=( $pattern )
	cd ${files[0]}
